...
export default function AssetRegistry() {
  ...
  const assets = Object.values(data.tasks).filter((t: any) => t.status);
            ...
            <tbody>
              {filteredAssets.map((asset) => (
                <tr key={asset.id} className='border-b border-white/5 hover:bg-white/5 transition-colors'>
                  ...
                  <td className='p-4 text-white font-bold'>{formatCurrency(asset.price * asset.quantity, config.localization.currency, config.localization.showAllCurrencies)}</td>
