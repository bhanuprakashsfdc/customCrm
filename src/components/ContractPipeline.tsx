...
export default function ContractPipeline() {
  ...
  const handleSave = (data: any) => {
    const record = {
      ...
      totalContractValue: parseFloat(data.value) || 0,
      startDate: data.startDate,
      endDate: data.endDate,
      ownerId: 'user_001',
    };
    saveRecord('contracts', record);
  };
            ...
            <tbody>
              {filteredContracts.map((contract) => (
                <tr key={contract.id} className='border-b border-white/5 hover:bg-white/5 transition-colors'>
                  ...
                  <td className='p-4 text-white'>{formatCurrency(contract.totalContractValue || 0, config.localization.currency, config.localization.showAllCurrencies)}</td>
